...
By coincidence, right after the Isenberg video, I came across [Nat Eliason on Peter Yang's channel](https://www.youtube.com/watch?v=nSBKCZQkmYw&t=1044s) talking about OpenClaw and a tool called qmd that's being built into it. qmd is a local markdown search engine. It does BM25 keyword search, same algorithm behind Elasticsearch, running locally with basically no overhead. I'm not using OpenClaw, but qmd works fine on its own as an MCP server. It indexes every markdown file in the repo and I can search it from Claude Code.
